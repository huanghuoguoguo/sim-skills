...
- 至少一个 workflow skill：`agent-check-report`
...
- `validate-spec` - 合并 structure/coverage，支持 --strict 标志
- `validate-report`
...
- `agent-check-report` - 检查 skipped_rules
...
- 段落（含 font_family_east_asia / font_family_ascii）
- 样式
- layout
...
- `validate-spec` - 合并 structure/coverage
- `validate-report`
...
### 任务 7：实现 `agent-check-report`

产出：

- skipped_rules 语义匹配逻辑
- 段落匹配策略（按 selector 语义）
- 属性比对逻辑
- agent_report.json 输出

关键要求：

- 能处理 frontmatter.abstract.zh.* 等语义化 selector
- 能处理 backmatter.references.* 等引用类 selector
- 输出置信度评分
...
5. 再收敛 `check-thesis` workflow 和报告出口
6. 最后实现 `agent-check-report`
...
- `agent-check-report` 能有效检查 skipped_rules
...
- 把 `validate-spec` 拆成多个 gate skills（已完成合并）
...
- Python 检查器明确输出 skipped_rules
- Agent 检查器针对 skipped_rules 进行语义匹配
- 保持两种检查模式的结果格式一致，便于下游消费
